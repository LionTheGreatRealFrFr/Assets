...
local SaveManager = {} do
    ...
    SaveManager.Parser = {
        Toggle = {
            ...
            Load = function(idx, data)
                -- FIX: Use getgenv().Toggles instead of Library.Toggles
                local object = getgenv().Toggles and getgenv().Toggles[idx]
        ...
        Slider = {
            ...
            Load = function(idx, data)
                -- FIX: Use getgenv().Options instead of Library.Options
                local object = getgenv().Options and getgenv().Options[idx]
                if object and object.Value ~= data.value then
                    object:SetValue(data.value)
                end
            end,
    ...
    function SaveManager:SetLibrary(library)
        ...
        -- FIX: Also sync Library.Toggles/Options to getgenv tables if they exist
        if getgenv().Toggles then
            self.Library.Toggles = getgenv().Toggles
        end
        if getgenv().Options then
            self.Library.Options = getgenv().Options
        end
    end
    ...
    function SaveManager:Save(name)
        ...
        -- FIX: Use getgenv().Toggles instead of self.Library.Toggles
        local toggles = getgenv().Toggles or (self.Library and self.Library.Toggles) or {}
        local options = getgenv().Options or (self.Library and self.Library.Options) or {}
